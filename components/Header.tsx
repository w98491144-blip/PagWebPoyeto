...
const Header = ({
  ...
  return (
    <header className="shadow-sm">
      ...
      <div className="site-header-band border-b border-ink-100 overflow-visible">
        <div className="container-shell flex flex-col gap-3 py-2 md:h-20 md:flex-row md:items-center md:justify-between md:py-0">
          <Link href="/" className="flex items-center gap-3">
            {settings.logo_url ? (
              <Image
                ...
                width={900}
                height={360}
                className="h-20 w-auto drop-shadow-md md:h-36 lg:h-44 -my-1 md:-my-3"
                unoptimized
              />
            ) : (
                ...
                <div className="nav-dropdown-panel group-hover:block group-focus-within:block">
                  {orderOptions.map((option) => (
                    <a
                      ...
                      className="nav-dropdown-item"
                      onClick={() =>
                        trackEvent("click_order_header", {
                          platform: option.provider
                        })
                      }
                    >
                      {option.label}
