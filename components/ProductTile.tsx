...
import { getPriceInfo, getTrackPriceAmount } from "@/lib/pricing";
...
const ProductTile = ({
  ...
  const handleOrderClick = (provider: "rappi" | "pedidosya") => {
    const productId = (product.slug || product.id || "").trim();
    const productName = product.name?.trim();
    const categoryValue = category?.trim();
    if (!productId || !productName || !categoryValue) return;
    const priceAmount = getTrackPriceAmount(product);
